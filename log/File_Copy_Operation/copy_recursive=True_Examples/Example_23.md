...
This example is designed to copy files from a source directory and its subdirectories to a destination directory, specifically targeting files that have a keyword in the suffix (in this case, **"US"**) and a specific file extension (in this case, **.csv**). The key steps are:
...
if not os.path.exists(source_directory):
    ...
    file_paths = glob(f'{source_directory}/**/*US.csv',recursive=True)
...
with pd.ExcelWriter("23_Example.xlsx",date_format = 'dd-mm-yyyy',datetime_format='dd-mm-yyyy') as writer:
