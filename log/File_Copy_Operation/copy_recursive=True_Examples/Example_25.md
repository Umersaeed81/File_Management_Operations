...
This example focuses on copying files from a source directory and its subdirectories to a destination directory based on whether the files start with a specific suffix (in this case, **"US"**), have a specific file extension (in this case, **.xlsx**), and are **smaller than 500KB in size**.  Here’s a detailed summary:
...
if not os.path.exists(source_directory):
    ...
    file_paths = [file for file in glob(f'{source_directory}/**/*US.xlsx',recursive=True) if os.path.getsize(file) < 500 * 1024]
...
with pd.ExcelWriter("25_Example.xlsx",date_format = 'dd-mm-yyyy',datetime_format='dd-mm-yyyy') as writer:
