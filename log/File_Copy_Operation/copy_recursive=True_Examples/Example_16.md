...
This example demonstrates filtering and copying files based on multiple keywords (in this case, **"US"** or **"AS"**) present in their filenames from a source directory and its subdirectories to a destination directory, including handling files within the directory. 
...
if not os.path.exists(source_directory):
    ...
    file_paths = set(file for pattern in patterns for file in glob(f'{source_directory}/**/{pattern}',recursive=True))
...
with pd.ExcelWriter("16_Example.xlsx",date_format = 'dd-mm-yyyy',datetime_format='dd-mm-yyyy') as writer:
