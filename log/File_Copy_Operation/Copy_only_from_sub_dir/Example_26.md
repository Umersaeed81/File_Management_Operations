...
This example focuses on copying files from the subdirectories of a specified source directory, but not from the source directory itself, to a destination directory based on whether the files end with a specific suffix (in this case, **"US"** or **"AS"**), have a specific file extension (in this case, **.txt** or **.xlsx**), and are **smaller than 500KB in size**.
...
with pd.ExcelWriter("26_Example.xlsx",date_format = 'dd-mm-yyyy',datetime_format='dd-mm-yyyy') as writer:
