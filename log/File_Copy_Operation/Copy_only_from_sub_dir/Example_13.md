...
The purpose of this code is to copy files from the subdirectories of a specified source directory, but not from the source directory itself, that have a suffix of **"US"** and a **size less than 500KB** to a target folder. Here’s a breakdown of its functionality:
...
if not os.path.exists(source_directory):
    ...
    file_paths = [file for file in glob(f'{source_directory}/**/*US.*',recursive=False) if os.path.getsize(file) < 500 * 1024]
...
with pd.ExcelWriter("13_Example.xlsx",date_format = 'dd-mm-yyyy',datetime_format='dd-mm-yyyy') as writer:
