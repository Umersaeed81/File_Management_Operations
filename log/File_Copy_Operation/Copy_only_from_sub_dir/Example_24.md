...
This example is designed to copy files from the subdirectories of a specified source directory, but not from the source directory itself, to a destination directory, specifically targeting files that have a keyword in the suffix (in this case, **"US"** or **"AS"**) and a specific file extension (in this case, **.xlsx**,**.txt**). The key steps are:
...
if not os.path.exists(source_directory):
    ...
    file_paths = [file
              ...
              for file in glob(f'{source_directory}/**/{base_pattern}{extension}',recursive=False)]
