...
# Copying Files from Specific Subdirectories to a Target Folder

The objective of this code is to copy all files (of any type) from specific subdirectories ('DA' and 'BH') of a specified source directory, but not from the source directory itself, to a target folder.
...
if not os.path.exists(source_directory):
    ...
    file_paths = [file for subdir in subdirectories 
...
```python
def get_file_info(path, subdirectories=None):
    # Get the list of files
    file_list = glob(f'{path}', recursive=True)

    # If subdirectories are provided, filter the file list
    if subdirectories:
        file_list = [file_path for file_path in file_list if any(sub in file_path for sub in subdirectories)]
...
# Define the specific subdirectories to filter for the source folder
subdirectories = ['DA', 'BH']

# Get file information from the source directory (with subdirectory filtering)
file_info_source_directory = get_file_info('D:/Copy/Umer_Saeed/**/*.*', subdirectories)
