...
# Copying Files Modified Within the Last 80 Hours

The objective of this code is to filter files from the source directory's subdirectories (excluding the main directory) that were modified within the last 80 hours and copy those files to a specified destination folder.
...
if not os.path.exists(source_directory):
    ...
    if datetime.fromtimestamp(os.path.getmtime(file)) > datetime.now() - timedelta(hours=80)]
