...
# Copying Files Based on Filename Length Including Extension

The objective of this code is to filter files from the subfolders of the source location, excluding the main folder, whose filenames (including extensions) have a length of 10 characters or less. These filtered files can then be copied to a specified destination folder.
...
if not os.path.exists(source_directory):
    ...
    if len(os.path.basename(file)) <= 10]
