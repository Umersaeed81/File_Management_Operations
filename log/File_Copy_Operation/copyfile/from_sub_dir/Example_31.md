...
The objective of this code is to check the existence of a source directory, filter files only from its subdirectories (excluding the main directory) whose filenames contain at least one numeric character (0-9), and copy those files to a specified destination folder.
...
if not os.path.exists(source_directory):
    ...
    file_paths = glob(f'{source_directory}/**/*[0-9]*.*',recursive=False)
