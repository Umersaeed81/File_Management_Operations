...
The objective of this code is to check the existence of a source directory, filter files from that directory whose filenames do not contain any numeric characters (0-9), and copy those files to a specified destination folder.
...
if not os.path.exists(source_directory):
    ...
    file_paths = [file for file in glob(f'{source_directory}/*.*') if not re.search(r'\d', os.path.basename(file))]
